	...
	void pickup() {
		if(Input.GetButtonDown("Fire1")||Input.GetButtonDown("Fire2")) {
			...
			if(Physics.Raycast(ray, out hit)) {
				Pickupable2 p = hit.collider.GetComponent<Pickupable2>();
				PickupableAn2 z = hit.collider.GetComponent<PickupableAn2> ();
				PickupableBer2 q = hit.collider.GetComponent<PickupableBer2>();
				if(p != null) {
					carrying = true;
